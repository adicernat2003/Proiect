...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Modifying
    @Transactional
    @Query(nativeQuery = true, value = "insert into student_undesired_accommodation values (null, ?1, ?2)")
    void insertRowIntoStudentUndesiredAccomodation(Long studentId, Long caminId);

    @Modifying
    @Transactional
    @Query(nativeQuery = true, value = "delete from student_undesired_accommodation sua where sua.student_id  = ?1")
    void deleteAllUndesiredCamineOfStudent(Long studentId);

    @Modifying
    @Transactional
    @Query(nativeQuery = true, value = "delete from student_undesired_accommodation sua where sua.student_id  = ?1 and sua.camin_id = ?2")
